...
BufferList* HashForBufferList::getBufferList(char *value)
{
  ...
  /**
  char *value_nul_delim = new char[value_size + 1]; // to add the trailing \0
...
void HashForBufferList::addBufferList(BufferList *list) 
{
  int size = list->getGroupBySize();
  // BUG FIX (see HASHNUL in BSortQBox)
  ...
  memcpy(group_by, list->getGroupByValues(), size);
  //char *orig_group_by = list->getGroupByValues(); // number of bytes

  /**
  char *group_by = new char[size + 1]; // to add the trailing \0
...
int HashForBufferList::getHashSize()
{
  return _hash_map.size();
}

// See above why this takes a size param
int HashForBufferList::getBufferListSize(char *value)
{
  BufferList *temp = getBufferList(value);
  if (temp != NULL)
    return temp->size();
  else
    return 0;
...
bool HashForBufferList::isEmpty()
{
